...
from models import db, User, TheoryBlock, TheoryTopic, TopicFile, Problem, ProblemFile, Message, RoleRequest, Favorite, Notification, GroupChat, GroupMessage, PinnedChat, group_member
...
def register():
    ...
    if request.method == 'POST':
        ...
        if not all([username, password, email, birth_date]):
            flash('Заполните все поля', 'error'); return render_template('register.html')
        if password != confirm:
            flash('Пароли не совпадают', 'error'); return render_template('register.html')
        if len(password) < 6:
            flash('Пароль минимум 6 символов', 'error'); return render_template('register.html')
        if '@' not in email:
            flash('Некорректный email', 'error'); return render_template('register.html')
        if not agreed:
            flash('Примите условия', 'error'); return render_template('register.html')
        if User.query.filter_by(username=username).first() or User.query.filter_by(email=email).first():
            flash('Логин или email занят', 'error'); return render_template('register.html')

        new_user = User(username=username, password=generate_password_hash(password), email=email, birth_date=birth_date, role='student', agreed=True)
        db.session.add(new_user); db.session.commit()
        login_user(new_user); flash('Регистрация успешна!', 'success')
        return redirect(url_for('index'))
...
def chat():
    ...
    same_role_users = User.query.filter(User.role == role, User.id != current_user.id).order_by(User.username).all() if role in ['operator', 'editor', 'admin'] else []
    fav_ids = {f.target_id for f in Favorite.query.filter_by(user_id=current_user.id).all()}

    system_groups = GroupChat.query.filter(GroupChat.is_custom == False, GroupChat.role_required.in_([role, 'admin' if role == 'super_admin' else role])).all()
    ...
    if group_id:
        ...
        if selected_group.is_custom and current_user not in selected_group.members:
            flash('Нет доступа', 'error'); return redirect(url_for('chat'))
    ...
    visible_online = set(online_users) if role == 'super_admin' else {uid for uid in online_users if db.session.get(User, uid) and db.session.get(User, uid).role != 'super_admin'} if role in ['admin', 'operator', 'editor'] else set()

    return render_template('chat.html', users=users, messages=messages, selected_user=db.session.get(User, chat_user_id) if chat_user_id else None, selected_group=selected_group, fav_ids=fav_ids, online_users=visible_online, my_groups=my_groups, same_role_users=same_role_users)
...
def pin_chat():
    ...
    if existing:
        ...
        db.session.add(PinnedChat(operator_id=current_user.id, user_id=target_id)); db.session.commit()
...
def create_group():
    ...
    group = GroupChat(name=name, description=desc, avatar=avatar_name, creator_id=current_user.id, role_required=current_user.role, is_custom=True)
    db.session.add(group); db.session.flush()
    members = [current_user] + [db.session.get(User, int(mid)) for mid in member_ids if db.session.get(User, int(mid)) and db.session.get(User, int(mid)).role == current_user.role and int(mid) != current_user.id]
    group.members.extend(members); db.session.commit()
    return jsonify({'ok': True, 'group_id': group.id})

@app.route('/notifications/mark_read', methods=['POST'])
@login_required
def mark_notifications_read():
    Notification.query.filter_by(user_id=current_user.id, is_read=False).update({'is_read': True}); db.session.commit()
    return jsonify({'ok': True})
...
def list_notifications():
    ...
    return jsonify([{'id': n.id, 'title': n.title, 'message': n.message, 'link': n.link, 'time': n.created_at.strftime('%H:%M'), 'read': n.is_read} for n in notifs])
...
def health():
    ...
    return {'status': 'ok' if db_ok else 'error', 'database': 'connected' if db_ok else 'disconnected', 'python_version': sys.version}, 200 if db_ok else 500

# ===== БЛОК 8: КОНТЕКСТ-ПРОЦЕССОР =====
...
def handle_join_chat(data):
    ...
    try: s, o = (int(s) if s else None), (int(o) if o else None)
    except: return
...
def handle_message(data):
    ...
    try: s, r = (int(s) if s else None), (int(r) if r else None)
    except: return
    ...
    with app.app_context():
        ...
        emit('new_message', {'sender_id': s, 'sender_name': u.username, 'content': c, 'timestamp': m.timestamp.strftime('%H:%M')}, room=f"chat_{min(s, r)}_{max(s, r)}")
        if r != s:
            notif = Notification(user_id=r, title=f'Новое сообщение от {u.username}', message=c[:100]+('...' if len(c)>100 else ''), link=url_for('chat', user_id=s))
            db.session.add(notif); db.session.commit()
            emit('new_notification', {'title': notif.title, 'message': notif.message, 'link': notif.link}, room=f'user_{r}')
...
def handle_group_msg(data):
    content = data.get('content', '').strip(); group_id = data.get('group_id')
    ...
    with app.app_context():
        ...
        emit('new_group_message', {'sender_name': current_user.username, 'content': content, 'timestamp': datetime.utcnow().strftime('%H:%M'), 'group_id': chat.id}, room=f"group_{chat.id}")

# ===== БЛОК 10: ЗАПУСК (в самом конце) =====
